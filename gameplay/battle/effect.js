class Laser{
	constructor(canvas, ctx, startPointX, startPointY, angleWidth, shootAngle, nearEnemy, owner){
		this.canvas = canvas;
		this.ctx = ctx;
	...
	drawLaser(newLine){
		if(newLine) {
			this.ctx.beginPath();
			this.ctx.moveTo(newLine.x, newLine.y);
			this.ctx.lineTo(newLine.x1, newLine.y1);
			this.ctx.closePath();
